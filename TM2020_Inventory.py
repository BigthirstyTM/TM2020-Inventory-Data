bl_info = {
    ...
    "description": "TM2020 Inventory - Select from search results and copy name from bottom bar.",
    "category": "Interface",
}
...
def draw_callback_px(self, context):
    ...
    tm_manager.current_visible_width = max((max_items * card_full_w) + margin, 600 * s) 
    ...
    # --- FOOTER TEXT LOGIC (FIXED) ---
    available_text_space = sx - tm_manager.ui_pos_x - (20 * s)
    
    # If a block is selected, show its name. Otherwise, show search results count or default text.
    if tm_manager.selected_block_name != "None":
        full_text = f"TM2020 | {tm_manager.selected_block_name}"
    elif show_search:
        full_text = f"RESULTS ({len(tm_manager.search_results)}) FOUND"
    else:
        full_text = "TM2020 | Select a block"
    ...
    if show_search:
        ...
        for i, item in enumerate(tm_manager.search_results):
            ...
            draw_card(rx, ry, item, i, (i == tm_manager.selected_search_idx), s, shader_flat, shader_img)
...
class VIEW3D_OT_tm_inventory(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'LEFTMOUSE' and event.value == 'PRESS':
            ...
            # COPY ON BAR (Now works with search results too)
                ...
                if not item_hit:
                    ...
                    if 0 <= r_idx < len(tm_manager.active_rows):
                        for i, _ in enumerate(tm_manager.active_rows[r_idx]):
                            ...
                            if ix < mx < ix + 105*s:
                                # Feature: clear search if root clicked
