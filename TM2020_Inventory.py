bl_info = {
    ...
    "description": "TM2020 Style Inventory - Stationary RMB Rotation & Deep Purge",
    "category": "Interface",
}
...
def update_theme(self, context):
    if self.theme_preset == 'DARK':
        ...
        self.ui_text_color = (1.0, 1.0, 1.0, 1.0); self.ghost_color = (0.2, 0.6, 1.0, 0.3); self.ghost_outline_color = (0.4, 0.8, 1.0, 1.0)
    elif self.theme_preset == 'LIGHT':
        self.ui_bg_color = (0.9, 0.9, 0.9, 0.95); self.ui_accent_color = (0.7, 0.7, 0.7, 1.0)
        self.ui_text_color = (0.05, 0.05, 0.05, 1.0); self.ghost_color = (1.0, 0.5, 0.0, 0.3); self.ghost_outline_color = (1.0, 0.4, 0.0, 1.0)
    elif self.theme_preset == 'CLASSIC':
        self.ui_bg_color = (0.01, 0.01, 0.01, 1.0); self.ui_accent_color = (0.0, 0.45, 0.2, 0.95)
        self.ui_text_color = (1.0, 1.0, 1.0, 1.0); self.ghost_color = (0.0, 1.0, 0.4, 0.2); self.ghost_outline_color = (0.2, 1.0, 0.4, 0.8)
...
class TM2020_Inventory_Preferences(bpy.types.AddonPreferences):
    ...
    ghost_color: FloatVectorProperty(name="Ghost Fill", subtype='COLOR', size=4, min=0.0, max=1.0, default=(0.0, 1.0, 0.4, 0.2))
...
class TM_Inventory_Manager:
    def __init__(self):
        ...
        self.is_ghosting = False; self.ghost_pos = Vector((0, 0, 0)); self.ghost_rot = 0
    ...
    def select_item(self, r_idx, i_idx, is_search=False):
        ...
        if is_folder and item.get("Childs"):
            ...
            self.is_ghosting = True; self.ghost_z_offset = 0.0; return True
...
def draw_3d_ghost(context, pos, rot_steps, fill_col, line_col, line_w):
    prefs = context.preferences.addons[__name__].preferences; shader = gpu.shader.from_builtin('UNIFORM_COLOR')
    sx, sy, sz = 32.0, 32.0, 8.0
    r = rot_steps * (math.pi/2); cos_r, sin_r = math.cos(r), math.sin(r)
    def rotate(vx, vy): return vx * cos_r - vy * sin_r, vx * sin_r + vy * cos_r
    
    # MATCH FIX: off_y = -1.0
    off_x, off_y = 0.0, -1.0 
    c = []
    for dx in [0.0+off_x, 1.0+off_x]:
        for dy in [0.0+off_y, 1.0+off_y]:
            for dz in [0.0, 1.0]:
                rx, ry = rotate(dx * sx, dy * sy); c.append((pos.x + rx, pos.y + ry, pos.z + dz * sz))
    
    gpu.state.blend_set('ALPHA'); indices = [(0,1,3,2), (4,5,7,6), (0,1,5,4), (2,3,7,6), (0,2,6,4), (1,3,7,5)]
    for face in indices:
    ...
    gv = [(pos.x + rotate(off_x*32,off_y*32)[0], pos.y + rotate(off_x*32,off_y*32)[1], 0), (pos.x + rotate((1+off_x)*32,off_y*32)[0], pos.y + rotate((1+off_x)*32,off_y*32)[1], 0),
          (pos.x + rotate((1+off_x)*32,off_y*32)[0], pos.y + rotate((1+off_x)*32,off_y*32)[1], 0), (pos.x + rotate((1+off_x)*32,(1+off_y)*32)[0], pos.y + rotate((1+off_x)*32,(1+off_y)*32)[1], 0),
          (pos.x + rotate((1+off_x)*32,(1+off_y)*32)[0], pos.y + rotate((1+off_x)*32,(1+off_y)*32)[1], 0), (pos.x + rotate(off_x*32,(1+off_y)*32)[0], pos.y + rotate((1+off_x)*32,(1+off_y)*32)[1], 0),
          (pos.x + rotate(off_x*32,(1+off_y)*32)[0], pos.y + rotate((1+off_x)*32,(1+off_y)*32)[1], 0), (pos.x + rotate(off_x*32,off_y*32)[0], pos.y + rotate(off_x*32,off_y*32)[1], 0)]
    batch_g = batch_for_shader(shader, 'LINES', {"pos": gv}); shader.bind(); shader.uniform_float("color", (1, 1, 1, 0.6)); gpu.state.line_width_set(1.5); batch_g.draw(shader)
...
def draw_callback_px(context):
    ...
    if tm_manager.is_hovering_help:
        ...
        blf.size(0, round(15 * s)); blf.color(0, 1, 1, 1, 1); lines = ["--- TM2020 INVENTORY HELP ---", "", "- L-Click Viewport: COMMIT (PLACE)", "- R-Click Viewport: ROTATE 90°", "- G-Key: TOGGLE GHOST MODE", "- Mouse-Wheel: Z-HEIGHT", "- Alt+Wheel: ZOOM TO GHOST", "- ESC: CLOSE ADDON"]
...
def draw_callback_view(context):
    if tm_manager.is_ghosting:
        p = context.preferences.addons[__name__].preferences; draw_3d_ghost(context, tm_manager.ghost_pos, tm_manager.ghost_rot, p.ghost_color, p.ghost_outline_color, p.ghost_outline_width)
...
class VIEW3D_OT_tm_inventory(bpy.types.Operator):
    ...
    def import_as_preview(self, context, name):
        ...
        if f:
            pre_colls = set(bpy.data.collections); pre_objs = set(bpy.data.objects)
    ...
    def sync_preview_pos(self):
        if tm_manager.active_preview_obj:
            ...
            tm_manager.active_preview_obj.rotation_euler.z = tm_manager.ghost_rot * (math.pi/2)
    ...
    def update_ghost_location(self, context, mx, my, sync_mouse=False):
        region, rv3d = context.region, context.space_data.region_3d
        if sync_mouse:
            # LOCK: ONLY warp mouse to established 3D position (no drift)
            new_2d = view3d_utils.location_3d_to_region_2d(region, rv3d, tm_manager.ghost_pos)
            if new_2d: self._is_warping = True; context.window.cursor_warp(int(new_2d.x + region.x), int(new_2d.y + region.y))
        else:
            # MOVE: Raycast to Z=0 and round to Corner
            ro = view3d_utils.region_2d_to_origin_3d(region, rv3d, (mx, my)); rd = view3d_utils.region_2d_to_vector_3d(region, rv3d, (mx, my))
            if abs(rd.z) > 0.0001:
                t = (tm_manager.ghost_z_offset - ro.z) / rd.z
                loc = ro + rd * t if t > 0 else ro + rd * 100
            else: loc = ro + rd * 100
            tm_manager.ghost_pos.x = round(loc.x / 32.0) * 32.0
            tm_manager.ghost_pos.y = round(loc.y / 32.0) * 32.0
    ...
    def modal(self, context, event):
        ...
        cur_w, bar_h, slot_w = tm_manager.current_bar_width, 35 * s, 115 * s
        hx, sx = tm_manager.ui_pos_x + cur_w - (bar_h * 2), tm_manager.ui_pos_x + cur_w - (bar_h * 2) - (280 * s)
        sy_cards, row_h = tm_manager.ui_pos_y + bar_h + 10*s, 145 * s
        ...
        if event.type == 'LEFTMOUSE' and event.value == 'PRESS':
            if in_ui:
                in_sb = sx <= mx <= hx and (tm_manager.ui_pos_y - bar_h) <= my <= tm_manager.ui_pos_y
                if tm_manager.is_searching and not in_sb: tm_manager.is_searching = False
        ...
        if event.type == 'RIGHTMOUSE' and event.value == 'PRESS':
            if in_ui:
                ...
                # STATIONARY ROTATION: Increment steps, freeze position, update object
                tm_manager.ghost_rot = (tm_manager.ghost_rot + 1) % 4
                self.sync_preview_pos(); return {'RUNNING_MODAL'}
            return {'PASS_THROUGH'}
        ...
        if event.type == 'ESC':
            self.cleanup_preview(); tm_manager.is_open = False
            bpy.types.SpaceView3D.draw_handler_remove(self._h2d, 'WINDOW'); bpy.types.SpaceView3D.draw_handler_remove(self._h3d, 'WINDOW'); return {'CANCELLED'}
        return {'PASS_THROUGH'}
