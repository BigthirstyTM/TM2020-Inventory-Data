bl_info = {
    ...
    "description": "Trackmania 2020 Style Inventory - Open the inventory with Ctrl-Shft-I",
    "category": "Interface",
}
...
class TM_Inventory_Manager:
    def __init__(self):
        ...
        self.ui_pos_x = 150
        self.ui_pos_y = 200
        self.ui_width = 1100 # Internal width used for scaling calculation
    ...
    def update_live_search(self):
        ...
        self.search_results = results[:50]
    ...
    def select_item(self, row_idx, item_idx, is_search_result=False):
        if is_search_result:
            ...
            self.selected_block_name = item.get("Name", "Unknown")
            return
...
def draw_callback_px(self, context):
    ...
    card_full_w = 105 * s + margin
    show_search = tm_manager.search_query != ""
    
    # Calculate Dynamic Width
    max_items_in_tree = max(len(row) for row in tm_manager.active_rows)
    search_row_items = min(len(tm_manager.search_results), max(1, int(tm_manager.ui_width // card_full_w))) if show_search else 0
    max_items = max(max_items_in_tree, search_row_items)
    content_width = (max_items * card_full_w) + margin
    tm_manager.current_visible_width = max(content_width, 600 * s) 

    search_rows_count = (len(tm_manager.search_results) // max(1, int(tm_manager.current_visible_width // card_full_w)) + 1) if show_search else 0
    total_h = (len(tm_manager.active_rows) + search_rows_count) * row_h
    
    # UI Render
    draw_rect(tm_manager.ui_pos_x, tm_manager.ui_pos_y, tm_manager.current_visible_width, total_h, (0, 0, 0, 0.4), shader_flat)
    ...
    # Resize handle (Fixed at the end of current_visible_width)
    ...
    blf.draw(0, f"{tm_manager.search_query}|" if tm_manager.is_searching else "Search...")
    
    # Footer with Copy info
    available_text_space = sx - tm_manager.ui_pos_x - (20 * s)
    full_text = f"TM2020 | {tm_manager.selected_block_name}" if not show_search else f"RESULTS ({len(tm_manager.search_results)})"
    ...
    for r_idx, row_items in enumerate(tm_manager.active_rows):
        ...
        for i, item in enumerate(row_items):
            rx = tm_manager.ui_pos_x + margin + i * card_full_w
    ...
    if show_search:
        ...
        cols = max(1, int(tm_manager.current_visible_width // card_full_w))
        for i, item in enumerate(tm_manager.search_results):
            rx = tm_manager.ui_pos_x + margin + (i % cols) * card_full_w
            ry = tm_manager.ui_pos_y + ((start_row_idx + (i // cols)) * row_h) + (10 * s)
            draw_card(rx, ry, item, i, False, s, shader_flat, shader_img)
...
class VIEW3D_OT_tm_inventory(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        show_search = tm_manager.search_query != ""
        search_rows = (len(tm_manager.search_results) // max(1, int(cur_w // (115*s))) + 1) if show_search else 0
        ...
        if tm_manager.is_searching:
            if event.value in {'PRESS', 'REPEAT'}:
                if event.type == 'BACKSPACE': 
                    tm_manager.search_query = tm_manager.search_query[:-1]
                    tm_manager.update_live_search(); return {'RUNNING_MODAL'}
                elif event.type == 'DEL': 
                    tm_manager.search_query = ""; tm_manager.update_live_search(); return {'RUNNING_MODAL'}
                elif event.ctrl and event.type == 'V': 
                    tm_manager.search_query += context.window_manager.clipboard
                    tm_manager.update_live_search(); return {'RUNNING_MODAL'}
                    ...
                    tm_manager.search_query += event.unicode
                    tm_manager.update_live_search(); return {'RUNNING_MODAL'}
        ...
        if event.type == 'LEFTMOUSE' and event.value == 'PRESS':
            ...
            if in_ui_body:
                ...
                if show_search:
                    start_row_idx = len(tm_manager.active_rows)
                    cols = max(1, int(cur_w // card_full_w))
                    for i, _ in enumerate(tm_manager.search_results):
                        ix = tm_manager.ui_pos_x + (10*s) + (i % cols) * card_full_w
                ...
                if not item_hit:
                    ...
                    if 0 <= r_idx < len(tm_manager.active_rows):
                        for i, _ in enumerate(tm_manager.active_rows[r_idx]):
                            ix = tm_manager.ui_pos_x + (10*s) + i * card_full_w
                            if ix < mx < ix + 105*s: tm_manager.select_item(r_idx, i); item_hit = True; break
        ...
        if event.type == 'MOUSEMOVE':
            ...
            if self.is_scaling: 
                # De breedte aanpassen op basis van de muis
                tm_manager.ui_width = max(600, mx - tm_manager.ui_pos_x)
                return {'RUNNING_MODAL'}
        if event.type == 'ESC':
